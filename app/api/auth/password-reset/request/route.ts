...
export async function POST(request: Request) {
  try {
    ...
    const users = await query<{ id: number }>('SELECT id FROM `User` WHERE email = ?', [email]);
    const user = users[0];
    ...
    if (user) {
      ...
      const resetUrlBase = process.env.PASSWORD_RESET_URL || 'http://localhost:5000/reset-password';
